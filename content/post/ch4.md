...
其中，$M\_0$ 为一个足够大的实数。观察可知，当$y^0(\\boldsymbol{x}) \+ \\boldsymbol{y}(\\boldsymbol{x})^\\top\\hat{\\boldsymbol{\\varepsilon}}\_\\omega\\le 0$ 时，$z\_\\omega$ 可取值 1，代表该约束在第 $\\omega$ 个场景中成立，否则不得不取值 0。
